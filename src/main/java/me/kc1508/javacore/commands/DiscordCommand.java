...
public class DiscordCommand implements CommandExecutor, TabCompleter {
    ...
    public DiscordCommand(FendorisPlugin plugin) {
        this.plugin = plugin;
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, String @NotNull [] args) {
        if (!(sender instanceof Player p)) {
            send(sender, "discord.only-player-message");
            return true;
        }
        ...
        if (!p.hasPermission("fendoris.operator")) {
            ...
            if (now < next) {
                ...
                if (remain < 1) {
                    send(p, "discord.cooldown-message-less-than-1");
                } else {
                    send(p, "discord.cooldown-message", "%seconds%", Long.toString(remain), "{seconds}", Long.toString(remain), "%s", "s");
                }
                return true;
            }
        ...
        send(p, "discord.message", "%player%", p.getName(), "{player}", p.getName());
        return true;
    }
    ...
    private void send(CommandSender target, String key, String... replacements) {
        ...
        if (raw == null || raw.isBlank()) {
            ...
            plugin.getLogger().warning("[Discord] Missing config key: " + key);
            return;
        }
        ...
        if (replacements.length % 2 == 0) {
            ...
            plugin.getLogger().warning("[Discord] Replacement array length is not even!");
        }
        ...
        try {
            ...
            target.sendMessage(FALLBACK);
            plugin.getLogger().warning("[Discord] Invalid MiniMessage at key '" + key + "': " + e.getMessage());
        }
    }
